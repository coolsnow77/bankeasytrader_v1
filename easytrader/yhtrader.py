...
class YHTrader(WebTrader):
    ...
    def login(self, throw=False):
        ...
        #print "login_page, ##############################################", self.config
        self.s = requests.session()
        self.s.headers.update(headers)
        data = self.s.get(self.config['login_page'])
        print "login_page, ##############################################", data
        ...
        if len(exchangeinfo) >= 2:
            for i in range(2):
                print "#########################shit ------------", exchangeinfo[i][TRADE_MARKET]
                if exchangeinfo[i][TRADE_MARKET][u'交易市场'] == u'深A':
    ...
    def post_login_data(self, verify_code):
        login_params = dict(
                ...
                #loginAccount=self.account_config['inputaccount'],
                trdpwd=self.account_config['trdpwd'],
                checkword=verify_code
                #verifyPassword=verify_code
        )
